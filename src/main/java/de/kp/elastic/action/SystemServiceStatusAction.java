...
import de.kp.elastic.SystemServiceStatusRequest;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class SystemServiceStatusAction extends BaseRestHandler {

	private String ACTION_NAME = "SystemServiceStatusAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public SystemServiceStatusAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/system/service/status", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			SystemServiceStatusRequest systemServiceStatusRequest = SystemServiceStatusRequest.parseRequest(parser);
			/*
			...
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getSystemServiceStatus(serviceName);
