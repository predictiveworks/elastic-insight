...
import de.kp.elastic.RegisterRequest;
import de.kp.elastic.StringResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class RegisterAction extends BaseRestHandler {

	private String ACTION_NAME = "RegisterAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public RegisterAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app/register", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			RegisterRequest registerRequest = RegisterRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = registerRequest.getNamespace();
			String appName = registerRequest.getAppName();
			String appVersion = registerRequest.getAppVersion();
			String blueprint = registerRequest.getBlueprint();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.registerApp(namespace, appName, appVersion, blueprint);
