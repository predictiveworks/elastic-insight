...
import de.kp.elastic.StartRequest;
import de.kp.elastic.StringResult;

import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class StartAction extends BaseRestHandler {

	private String ACTION_NAME = "StartAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public StartAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app/start", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			StartRequest startRequest = StartRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = startRequest.getNamespace();
			String appName = startRequest.getAppName();
			String appVersion = startRequest.getAppVersion();
			
			String progName = startRequest.getProgName();
			String progType = startRequest.getProgType();
			
			Map<String,String> runtimeArgs = startRequest.getRuntimeArgs();
			
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.startProg(namespace, appName, appVersion, progName, progType, runtimeArgs);
