...
import de.kp.elastic.ExistsRequest;
import de.kp.elastic.StringResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class ExistsAction extends BaseRestHandler {

	private String ACTION_NAME = "ExistsAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public ExistsAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app/exists", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			ExistsRequest existsRequest = ExistsRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = existsRequest.getNamespace();
			String appName = existsRequest.getAppName();
			String appVersion = existsRequest.getAppVersion();

			Boolean exists = AccessController.doPrivileged((PrivilegedAction<Boolean>) () -> {
				return job.existsApp(namespace, appName, appVersion);

			});

			StringResult result = new StringResult(exists.toString());
			return result;
