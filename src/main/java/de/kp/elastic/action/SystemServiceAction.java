...
import de.kp.elastic.SystemServiceRequest;
import de.kp.elastic.SystemServiceResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class SystemServiceAction extends BaseRestHandler {

	private String ACTION_NAME = "SystemServiceAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public SystemServiceAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/system/service", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				SystemServiceResult response = doRequest(request);
	...
	private SystemServiceResult doRequest(RestRequest request) throws Exception {

		if (request.hasContentOrSourceParam()) {

			XContentParser parser = request.contentOrSourceParamParser();
			SystemServiceRequest systemServiceRequest = SystemServiceRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String serviceName = systemServiceRequest.getServiceName();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getSystemServiceLiveInfoAsJson(serviceName);

			});

			SystemServiceResult result = new SystemServiceResult(json);
			return result;
