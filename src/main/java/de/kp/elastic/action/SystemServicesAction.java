...
import de.kp.elastic.SystemServicesResult;
...
public class SystemServicesAction extends BaseRestHandler {
	...
	public SystemServicesAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/system/services", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				SystemServicesResult response = doRequest(request);
	...
	private SystemServicesResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			SystemServicesResult result = new SystemServicesResult(json);
			return result;
