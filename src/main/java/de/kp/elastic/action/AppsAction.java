...
import de.kp.elastic.AppsRequest;
import de.kp.elastic.AppsResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

import static org.elasticsearch.rest.RestRequest.Method.POST;

public class AppsAction extends BaseRestHandler {

	private String ACTION_NAME = "AppsAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public AppsAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/apps", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				AppsResult response = doRequest(request);
	...
	private AppsResult doRequest(RestRequest request) throws Exception {

		if (request.hasContentOrSourceParam()) {

			XContentParser parser = request.contentOrSourceParamParser();
			AppsRequest appsRequest = AppsRequest.parseRequest(parser);
			/*
			 * Extract request parameter and delegate execution to CDAPJob which
			 * returns the list of registered applications
			 */
			String namespace = appsRequest.getNamespace();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getAppsAsJson(namespace);

			});

			AppsResult result = new AppsResult(json);
			return result;
