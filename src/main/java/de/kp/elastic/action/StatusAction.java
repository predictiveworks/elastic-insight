...
import de.kp.elastic.StatusRequest;
import de.kp.elastic.StringResult;

import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class StatusAction extends BaseRestHandler {

	private String ACTION_NAME = "StatusAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public StatusAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app/status", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			StatusRequest statusRequest = StatusRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = statusRequest.getNamespace();
			String appName = statusRequest.getAppName();
			String appVersion = statusRequest.getAppVersion();
			
			String progName = statusRequest.getProgName();
			String progType = statusRequest.getProgType();
			
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getProgStatus(namespace, appName, appVersion, progName, progType);
