...
import de.kp.elastic.StopRequest;
import de.kp.elastic.StringResult;

import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class StopAction extends BaseRestHandler {

	private String ACTION_NAME = "StopAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public StopAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app/stop", this);
		/*
	...
	private StringResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			StopRequest stopRequest = StopRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = stopRequest.getNamespace();
			String appName = stopRequest.getAppName();
			String appVersion = stopRequest.getAppVersion();
			
			String progName = stopRequest.getProgName();
			String progType = stopRequest.getProgType();
			
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.stopProg(namespace, appName, appVersion, progName, progType);
