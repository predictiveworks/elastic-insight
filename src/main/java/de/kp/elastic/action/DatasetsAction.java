...
import de.kp.elastic.DatasetsRequest;
import de.kp.elastic.DatasetsResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

import static org.elasticsearch.rest.RestRequest.Method.POST;

public class DatasetsAction extends BaseRestHandler {

	private String ACTION_NAME = "DatasetsAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public DatasetsAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/datasets", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				DatasetsResult response = doRequest(request);
	...
	private DatasetsResult doRequest(RestRequest request) throws Exception {

		if (request.hasContentOrSourceParam()) {

			XContentParser parser = request.contentOrSourceParamParser();
			DatasetsRequest datasetRequest = DatasetsRequest.parseRequest(parser);
			/*
			 * Extract request parameter delegate execution to CDAPJob which
			...
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getDatasetsAsJson(namespace);

			});

			DatasetsResult result = new DatasetsResult(json);
			return result;
