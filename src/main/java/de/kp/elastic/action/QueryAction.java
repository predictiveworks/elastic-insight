...
import de.kp.elastic.QueryRequest;
import de.kp.elastic.QueryResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

import static org.elasticsearch.rest.RestRequest.Method.POST;

public class QueryAction extends BaseRestHandler {

	private String ACTION_NAME = "QueryAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public QueryAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/dataset/query", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				QueryResult response = doRequest(request);
	...
	private QueryResult doRequest(RestRequest request) throws Exception {

		if (request.hasContentOrSourceParam()) {

			XContentParser parser = request.contentOrSourceParamParser();
			QueryRequest queryRequest = QueryRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob which
			 * returns the query result
			 */
			String namespace = queryRequest.getNamespace();
			String datasetName = queryRequest.getDatasetName();
			String query = queryRequest.getQuery();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.executeQueryAsJson(namespace,datasetName,query);

			});

			QueryResult result = new QueryResult(json);
			return result;
