...
public class MetricsAction extends BaseRestHandler {

	private String ACTION_NAME = "MetricsAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public MetricsAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/metrics/search", this);
		/*
	...
	private JsonResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			MetricsRequest metricsRequest = MetricsRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = metricsRequest.getNamespace();
			Map<String,String> context = metricsRequest.getContext();
			
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getMetricsAsJson(namespace, context);
