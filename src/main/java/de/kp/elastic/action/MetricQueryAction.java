...
import de.kp.elastic.MetricQueryResult;
import de.kp.elastic.MetricQueryRequest;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class MetricQueryAction extends BaseRestHandler {

	private String ACTION_NAME = "MetricQueryAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public MetricQueryAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/metrics/query", this);
		/*
	...
	protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
		...
		return channel -> {
			try {
				...
				MetricQueryResult response = doRequest(request);
	...
	private MetricQueryResult doRequest(RestRequest request) throws Exception {

		if (request.hasContentOrSourceParam()) {

			XContentParser parser = request.contentOrSourceParamParser();
			MetricQueryRequest queryRequest = MetricQueryRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String metric = queryRequest.getMetric();
			Map<String,String> tags = queryRequest.getTags();
			
			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.queryMetricAsJson(tags, metric);

			});

			MetricQueryResult result = new MetricQueryResult(json);
			return result;
