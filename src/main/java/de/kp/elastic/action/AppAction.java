...
import de.kp.elastic.AppRequest;
import de.kp.elastic.AppsResult;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class AppAction extends BaseRestHandler {

	private String ACTION_NAME = "AppAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public AppAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/app", this);
		/*
	...
	private AppsResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			AppRequest appRequest = AppRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = appRequest.getNamespace();
			String appName = appRequest.getAppName();
			String appVersion = appRequest.getAppVersion();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getAppAsJson(namespace, appName, appVersion);
