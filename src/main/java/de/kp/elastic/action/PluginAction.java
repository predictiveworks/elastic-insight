...
import de.kp.elastic.PluginRequest;
import de.kp.elastic.cdap.CDAPConf;
import de.kp.elastic.cdap.job.CDAPJob;

public class PluginAction extends BaseRestHandler {

	private String ACTION_NAME = "PluginAction";
	private String CONFIG_PATH = "/application.conf";

	private CDAPJob job;

	public PluginAction(Settings settings, RestController controller) {
		super(settings);
		controller.registerHandler(POST, "/_cdap/plugins", this);
		/*
	...
	private JsonResult doRequest(RestRequest request) throws Exception {
		...
		if (request.hasContentOrSourceParam()) {
			...
			PluginRequest pluginRequest = PluginRequest.parseRequest(parser);
			/*
			 * Extract request parameters and delegate execution to CDAPJob
			 */
			String namespace = pluginRequest.getNamespace();

			String json = AccessController.doPrivileged((PrivilegedAction<String>) () -> {
				return job.getPluginsAsJson(namespace);
